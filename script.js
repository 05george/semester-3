...
const debouncedCleanupHover = debounce(() => {
...
function lazyLoadImageWithProgress(imgElement, weekNumber) {
    ...
    xhr.onprogress = event => {
        if (event.lengthComputable && text) {
    ...
    while (current && current.tagName !== 'svg') {
        if (current.tagName === 'g') {
            ...
            if (images.length) {
                ...
                return { src: imageSource, width: parseFloat(baseImage.getAttribute('width')), height: parseFloat(baseImage.getAttribute('height')), group: current };
            }
        }
        current = current.parentNode;
    }
...
function startHover() {
    ...
    if (!imageElement || !imageElement.getAttribute('href')) return;
