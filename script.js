window.onload = function() {
    ...
    async function fetchGithubFiles(path = "rrs") {
        try {
            const response = await fetch(`https://api.github.com/repos/05george/semester-3/contents/${path}`);
            if (!response.ok) return [];
            return await response.json();
        } catch (error) {
            console.error("خطأ في جلب ملفات API:", error);
            return [];
        }
    }

    // --- تحديث واجهة قائمة الخشب (GitHub API Version) ---  
    async function updateWoodInterface() {  
        ...
        // تحديد المسار (المجلد rrs هو البداية الافتراضية)
        const pathToShow = currentFolder === "" ? "rrs" : currentFolder; 
        const items = await fetchGithubFiles(pathToShow);

        // تصفية الملفات (مجلدات أو ملفات PDF)
    ...
    // --- الدوال المساعدة للتنسيق والحركة ---  
