...
if(baseText){baseText.style.opacity='0';
    ...
    const zoomText=document.createElementNS('http://www.w3.org/2000/svg','foreignObject');
    zoomText.setAttribute('x', absoluteX);
    zoomText.setAttribute('y', absoluteY);
    zoomText.setAttribute('width', width);
    zoomText.setAttribute('height', height);
    zoomText.style.opacity = '1';

    const zoomDiv = document.createElement('div');
    zoomDiv.style.width = '100%';
    zoomDiv.style.height = '100%';
    zoomDiv.style.backgroundColor = 'black';
...
allRectsAndLabels.forEach(element=>{let match=false;
    ...
    if(element.tagName==='rect' && element.matches('.image-mapper-shape')){
        ...
        textContentToMatch = label ? label.getAttribute('data-original-text') : '';
        if(textContentToMatch.toLowerCase().includes(lowerQuery)){match=true}
    }else if(element.tagName==='foreignObject' && element.matches('.rect-label')){
        textContentToMatch = element.getAttribute('data-original-text') || '';
        if(textContentToMatch.toLowerCase().includes(lowerQuery)){match=true}
    }
    ...
    if (element.tagName === 'rect' && element.matches('.image-mapper-shape')) {
        rectShape = element;
        foreignObject = element.previousElementSibling;
    } else if (element.tagName === 'foreignObject' && element.matches('.rect-label')) {
        foreignObject = element;
        rectShape = element.nextElementSibling;
    }
    ...
    if(rectShape){
        if(query.length>0&&!match){
            ...
            if(foreignObject)foreignObject.style.opacity='0.1';
        }else{
            rectShape.style.opacity='1';
            rectShape.style.pointerEvents='auto';
            if(foreignObject)foreignObject.style.opacity='1';
            if(query.length>0&&match){
