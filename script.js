window.onload = function () {
    ...
    // 4. التعامل مع الـ Hover والـ Zoom
    ...
    function cleanupHover() {
        ...
        const { rect, clipPathId, zoomPart, zoomText, baseText, animationId } = activeState;
        if (animationId) clearInterval(animationId);
        rect.style.filter = 'none';
        if (zoomPart) zoomPart.style.filter = 'none';
        if (zoomText) zoomText.style.filter = 'none';
    ...
    function attachHover(rect, i) {
        ...
        if (!isTouchDevice) {
            rect.addEventListener('mouseover', function () { if (interactionEnabled) startHover.call(rect) });
            rect.addEventListener('mouseout', function () { if (interactionEnabled) stopHover.call(rect) });
        }
        ...
        allRectsAndLabels.forEach(element => {
            ...
            if (rectShape && rectShape.matches('rect.image-mapper-shape')) {
                ...
                if (query.length > 0 && !match) {
                    ...
                    } else { rectShape.style.filter = 'none' }
                }
            }
        })
    }
    ...
    // 8. مراقبة العناصر الجديدة (MutationObserver)
    ...
    // 9. التوجيهات اليدوية (UI)
    jsToggle.addEventListener('change', function () {
        interactionEnabled = this.checked;
        const label = document.getElementById('toggle-label');
        if (interactionEnabled) { label.textContent = 'Interaction Enabled' }
        else {
